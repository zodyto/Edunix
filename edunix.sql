...
--
-- Stand-in structure for view `age_calc`
...
--
DROP TABLE IF EXISTS `age_calc`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `age_calc`  AS SELECT `student`.`student_id` AS `student_id`, floor((to_days(current_timestamp()) - to_days(`student`.`dob`)) / 365) AS `Age` FROM `student` ;
